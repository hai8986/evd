  ...
  const { data: stats, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from('clients').select('id, active, wallet_balance, institution_name', { count: 'exact' }),
        supabase.from('projects').select('id, status, payment_status, total_amount, paid_amount, product_id, quantity, name', { count: 'exact' }),
        ...
        supabase.from('products').select('id, active, category, name', { count: 'exact' }),
        supabase.from('templates').select('id, is_public, vendor_id, name', { count: 'exact' }),
        ...
        supabase.from('wallet_transactions').select('id, amount, transaction_type', { count: 'exact' }),
      ]);
      ...
      // Projects without design templates (in early stages)
      const projectsWithoutTemplate = projects.filter(p => p.status === 'draft' || p.status === 'data_upload').length;

      // Sub-projects count (group by product categories)
      ...
      // Calculate ID Card specific stats (assuming ID Card is a product category)
      const idCardStats = {
        total: dataRecords.length,
        inProcess: recordsByStatus.processing,
        readyForPrint: projects.filter(p => p.status === 'approved').length,
        printing: projects.filter(p => p.status === 'printing').length,
        printed: 0, // Would need a separate field
        pendingDelivery: projects.filter(p => p.status === 'dispatched').length,
        delivered: projects.filter(p => p.status === 'delivered').length,
      };
      ...
      return {
        ...
        projects: { 
          ...
          withoutTemplate: projectsWithoutTemplate, 
          totalQuantity,
          subProjects: productCategories.length,
        },
        ...
        products: { total: products.length, active: activeProducts, categories: productCategories },
